...
module.exports = {
	...
	push: function(modulePath, pushHost) {
		...
		var compress = new targz().compress(
			...
			var saskaviId = fs.existsSync('.saskavi-id') ?
				fs.readFileSync('.saskavi-id').toString() : null;

			if (saskaviId) {
				console.log("    Reusing deploy id:".yellow, saskaviId);
				headers["x-saskavi-id"] = saskaviId;
			}
			...
			fs.createReadStream('/tmp/saskavi-deployment.tar.gz').pipe(
				request.post({
					...
					function(err, r, body) {
						...
						if (rr.status) {
							fs.writeFileSync(".saskavi-id", rr.id);
	...
	kill: function(modulePath, pushHost) {
		...
		if (!saskaviId)
			return console.log("No .saskavi-id file found in current directory, don't know what to kill".red);

		console.log("    Killing saskavi:".yellow, saskaviId);
		...
		request.post({
			...
			function(err, r, body) {
				...
				var rr = JSON.parse(body);
				if (rr.status)
					return fs.unlink(".saskavi-id", function() {
						console.log("Saskavi killed.".green);
					});
